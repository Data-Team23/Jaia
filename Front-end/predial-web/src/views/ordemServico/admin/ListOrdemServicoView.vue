<template>
  <div class="container">
      ...
      <div class="list-container">
        ...
        <div class="table-container">
          <table>
            ...
            <tbody
              v-for="(OrdemServico, index) in paginatedOrdemServicos"
              :key="index"
            >
              <tr>
                <td>{{ OrdemServico.no }}</td>
                ...
                <td>
                  {{
                    OrdemServico.id_check.departamento.nome ?? "Não informado"
                  }}
                </td>
                <td>
                  <span
                    ...
                    id="edit-button"
                    @click="editOrdemServico(OrdemServico.id)"
                  >
                    edit
                  </span>
                  <span
                    class="material-symbols-outlined"
                    id="aprove-button"
                    @click="aproveOrdemServico(OrdemServico.id)"
                  >
                    zoom_out_map
                  </span>
                  <span
                    class="material-symbols-outlined"
                    id="delete-button"
                    @click="deletOrdem(OrdemServico.id)"
                  >
                    delete
                  </span>
                </td>
    ...
    <v-dialog v-model="aproveDialog" width="80%">
      <div class="add-client-container">
        <div class="close-button">
          <span class="material-symbols-outlined" @click="aproveDialog = false">
            close
          </span>
        </div>
    ...
    <v-dialog v-model="deleteDialog" width="30%">
      <div class="delete-client-container">
        ...
        <div class="confirm-delete-button">
          <InputButton text-button="Sim" @click="deleteOrdem()"></InputButton>
          <InputButton
            text-button="Não"
...
function filterOrdemServicos() {
  filteredOrdemServicos.value = OrdemServicos.value.filter(
    (OrdemServico: any) => {
      const selectedValue = OrdemServico[selectedFilter.value];
      totalPages = computed(() =>
        Math.ceil(filteredOrdemServicos.value.length / itemsPerPage.value)
      );
...
function deleteOrdem() {
  ...
  axios
    .delete(`http://localhost:8080/ordem-servico/${id}`)
    .then((response) => {
    ...
    .catch((error) => {
      window.alert("Erro ao excluir o Ordem");
      deleteDialog.value = false;
    });
}
...
const paginate = () => {
  ...
  paginatedOrdemServicos.value = filteredOrdemServicos.value.slice(
    startIndex,
    endIndex
  );
};
...
let totalPages = computed(() =>
  Math.ceil(OrdemServicos.value.length / itemsPerPage.value)
);
